...
class TestIntegrationGithubOrgClient(TestCase):
    """
    Create the TestIntegrationGithubOrgClient(unittest.TestCase) class and
    implement the setUpClass and tearDownClass which are part of the
    unittest.TestCase API.

    Use @parameterized_class to decorate the class and parameterize it with
    fixtures found in fixtures.py. The file contains the following fixtures:

    org_payload, repos_payload, expected_repos, apache2_repos
    The setupClass should mock requests.get to return example payloads found
    in the fixtures.

    Use patch to start a patcher named get_patcher, and use side_effect to make
    sure the mock of requests.get(url).json() returns the correct fixtures for
    the various values of url that you anticipate to receive.

    Implement the tearDownClass class method to stop the patcher.
    """
    @classmethod
    def setUpClass(cls) -> None:
        """setup class before tests are ran"""
        mock_payload = {
        ...
        def get_payload(url):
            if url in mock_payload:
                return Mock(**{'json.return_value': mock_payload[url]})
            return HTTPError
    ...
    def tearDownClass(cls) -> None:
        """deletes class after tests are ran"""
